...
def main():
    ...
    try:
        for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
            ...
            # ultraSonic Distance
            distance = 30
            distance_curr = ultrasonic_sensor.checkdist() * 100
            # print(distance)
            acc = (-1) * pidVel.update(distance,distance_curr,time.time())
            if vel <0:
                 motor_control.move(abs(vel), 'backward')  

            # control_Vel = -pidVel.update(10, distance, time.time())
            # print(distance)
            if distance_curr <= 20 and obstacleFlag == 0 and vehicleDetectFlag == 1:
                obstacleFlag = 1
